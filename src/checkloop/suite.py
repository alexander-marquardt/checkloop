...
def _print_clone_review_via_claude(
    *,
    ...
    review_prompt = (
        ...
        f"consistent with this project's conventions. Read the project's "
        f"standards documents first — CLAUDE.md, AGENTS.md, CONTRIBUTING.md, "
        f".cursorrules, and any similar policy file at the repo root (e.g. "
        f"STYLEGUIDE.md, ENGINEERING.md). CONTRIBUTING.md in particular often "
        f"carries the rules that matter most for adopting a review: the "
        f"test-with-every-behaviour-change rule, the no-AI-attribution rule, "
        f"commit-message style, and the no-net-neutral-churn rule. If none "
        f"of those files exist, skim README.md's contributor / development / "
        f"testing sections for equivalent guidance. Skip anything that looks "
        f"wrong, over-eager, "
        f"or lower quality than what is already here. For each change worth "
        f"acting on, open a GitHub issue first (`gh issue create`) that records "
        f"the problem and a pointer to checkloop's take on it — the clone at "
        f"{clone_dir}, the scratch branch {scratch_branch}, and the relevant "
        f"commit SHA(s), files, and tests, with the command to read them "
        f"(`git -C {clone_dir} show <sha>`). Then solve that issue "
        f"independently by editing this repo directly: treat checkloop's commit "
        f"as a REFERENCE IMPLEMENTATION and a second opinion — one way the issue "
        f"was solved, plus the regression tests it thought worth writing — NOT "
        f"material to merge, cherry-pick, or port. Build your own solution (and "
        f"any sub-agent you spawn builds its own) on one or more new branches "
        f"off {pr_base} with clear names and descriptive commit messages that "
        f"carry the issue number — group related work together — then COMPARE "
        f"your implementation and your tests against checkloop's reference: did "
        f"it cover an edge case or write a test you missed, or did you find a "
        f"cleaner or more correct approach than it took? The reference exists "
        f"for that cross-check, so the work is not lost — but your solution must "
        f"stand on its own, not be a copy of checkloop's. Run the project's "
        f"tests and linters on each new branch. If a branch passes and is a "
        f"genuine improvement, push it, open a PR, and merge it through this "
        f"repo's normal workflow. All pushes/PRs/merges happen from this repo, "
        f"never from {clone_dir}. Keep the PR title and body free of AI or tool "
    ...
    sections = [
        "**Accepted** — for each issue you acted on, one bullet naming the "
        "issue number, the file(s) or area touched, the new branch it landed "
        "on, a one-line reason it was worth doing, and how your solution "
        "compared to checkloop's reference (matched it, improved on it, or "
        "caught something it missed)",
