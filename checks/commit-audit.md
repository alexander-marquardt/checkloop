...
2. **Classify each commit as exactly one of these categories:**
   ...
   - **E — Readability or maintainability win.** A meaningful clarity improvement: a rename whose old name was actually confusing, a flattened deep nesting, a helper extracted with at least two real callers, a removed dead branch. No new test needed if existing coverage still exercises the behavior. A rename that fixes a name that **lies** (its value contradicts the identifier) or is **overloaded** (one name reused for different concepts across contexts or layers, or one concept under several names) is category E even when the commit is rename-only — it removes a correctness hazard, not just stylistic noise; do NOT downgrade it to G.
   ...
   - **G — NET-NEUTRAL CHURN.** The commit makes the code different without making it better: a rename for marginal clarity, a reordering with no semantic effect, an abstraction with one caller, a deleted docstring or comment whose deletion did not improve readability. Zero behavior change, zero readability win for a future reader. The reviewer should drop or fold this commit. Apply the rename-revert test: if reverting every identifier rename in the commit leaves zero substantive content, classify G regardless of how much clearer the new names read — identifier-rename-only commits are the most common pattern reviewers reject. **Exception:** a rename-only commit that fixes a name that *lies* or is *overloaded* (category E above) is NOT G — disambiguating such a name is a correctness fix that has repeatedly shipped as bugs when skipped, so the rename-revert test does not apply to it. Reserve G for renames between two names that are *both* accurate and unambiguous.
