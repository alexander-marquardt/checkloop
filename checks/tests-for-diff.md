...
6. **If a real behavior change cannot be tested in this stack** (no framework, no fixture for the involved subsystem, behavior only observable through a UI that has no end-to-end rig), do NOT skip silently. Note the unit, the file, the reason, and emit a `# TODO: regression test for <unit>` comment near the change so the gap is visible in the diff.
