...
Do NOT commit lockfile refreshes (`package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `uv.lock`, `poetry.lock`, `Cargo.lock`, `go.sum`) whose only effect is to "align" peer-attribution flags or reorder transitive deps to match what the resolver currently produces. These changes flip back and forth across runs and contribute no signal — the underlying cause is upstream resolver instability, not a real fix. Lockfile changes are only landable when paired with a deliberate manifest change (`package.json`, `pyproject.toml`, `go.mod`, `Cargo.toml`) in the same commit. If a tool you ran during this check touched the lockfile incidentally, revert that hunk before committing.
