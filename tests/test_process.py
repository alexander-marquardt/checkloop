...
class TestStreamDescendantScanning:
    """Tests for periodic descendant tree scanning in _stream_process_output()."""

    def test_accumulates_descendants_on_interval(self) -> None:
        """After the scan interval elapses, descendant PIDs are collected."""
        mock_proc = mock.MagicMock()
        mock_proc.stdout = io.BytesIO(b"")
        mock_proc.pid = 100

        acc: set[int] = set()
        # Simulate time advancing past _MEMORY_CHECK_INTERVAL on second call
        time_values = iter([0.0, 0.0, 0.0, 20.0])

        with mock.patch.object(process, "_check_idle_timeout", return_value=False), \
             mock.patch.object(process, "_check_hard_timeout", return_value=False), \
             mock.patch.object(process, "_check_memory_limit", return_value=(False, 0.0)), \
             mock.patch.object(process, "find_all_descendant_pids", return_value=[200, 300]) as mock_find, \
             mock.patch("time.time", side_effect=lambda: next(time_values, 20.0)):
            process._stream_process_output(
                mock_proc, idle_timeout=120, debug=False,
                check_timeout=0, max_memory_mb=0,
                accumulated_descendant_pids=acc,
            )
        assert {200, 300}.issubset(acc)
        mock_find.assert_called_with(100)

    def test_no_accumulation_when_none_passed(self) -> None:
        """When accumulated_descendant_pids is None, no tree scanning occurs."""
        mock_proc = mock.MagicMock()
        mock_proc.stdout = io.BytesIO(b"")
        mock_proc.pid = 100
        ...
        with mock.patch.object(process, "_check_idle_timeout", return_value=False), \
            ...
            process._stream_process_output(
                mock_proc, idle_timeout=120, debug=False,
                check_timeout=0, max_memory_mb=0,
                accumulated_descendant_pids=None,
            )
...
class TestKillRemainingDescendants:
    """Tests for _kill_remaining_descendants() escaped-process helper."""

    def setup_method(self) -> None:
        from checkloop.monitoring import previous_descendant_pids
        self._saved = set(previous_descendant_pids)
        previous_descendant_pids.clear()

    def teardown_method(self) -> None:
        from checkloop.monitoring import previous_descendant_pids
        previous_descendant_pids.clear()
        previous_descendant_pids.update(self._saved)

    def test_kills_accumulated_pids(self) -> None:
        from checkloop.monitoring import previous_descendant_pids
        with mock.patch.object(process, "kill_pids", return_value=2) as mock_kill:
            process._kill_remaining_descendants({100, 200, 300}, root_pid=300)
            # root_pid (300) must be excluded from targets
            called_pids = mock_kill.call_args[0][0]
            assert 300 not in called_pids
            assert set(called_pids) == {100, 200}
        # Survivors tracked for follow-up sweeps
        assert {100, 200}.issubset(previous_descendant_pids)

    def test_excludes_own_pid(self) -> None:
        import os
        my_pid = os.getpid()
        with mock.patch.object(process, "kill_pids", return_value=1) as mock_kill:
            process._kill_remaining_descendants({my_pid, 500}, root_pid=999)
            called_pids = mock_kill.call_args[0][0]
            assert my_pid not in called_pids
            assert 500 in called_pids

    def test_empty_set_is_noop(self) -> None:
        with mock.patch.object(process, "kill_pids") as mock_kill:
            process._kill_remaining_descendants(set(), root_pid=123)
            mock_kill.assert_not_called()

    def test_only_root_pid_is_noop(self) -> None:
        """If all_known contains only root_pid, nothing to kill."""
        with mock.patch.object(process, "kill_pids") as mock_kill:
            process._kill_remaining_descendants({123}, root_pid=123)
            mock_kill.assert_not_called()

    def test_tracks_all_targets_for_followup(self) -> None:
        """Even PIDs that fail to kill are tracked in previous_descendant_pids."""
        from checkloop.monitoring import previous_descendant_pids
        with mock.patch.object(process, "kill_pids", return_value=0):
            process._kill_remaining_descendants({400, 500}, root_pid=999)
        assert {400, 500}.issubset(previous_descendant_pids)


class TestKillProcessGroupDescendants:
    """Tests for descendant PID handling in _kill_process_group()."""

    def test_merges_extra_pids_with_fresh_scan(self) -> None:
        """extra_pids from streaming are merged with a fresh tree snapshot."""
        mock_proc = mock.MagicMock()
        mock_proc.pid = 100
        with mock.patch("os.getpgid", return_value=100), \
             ...
             mock.patch.object(process, "_kill_remaining_descendants") as mock_kill_desc:
            process._kill_process_group(mock_proc, extra_pids={400})
            mock_find.assert_called_once_with(100)
            # all_known should be {200, 300, 400}
            call_args = mock_kill_desc.call_args
            assert call_args[0][0] == {200, 300, 400}
            assert call_args[0][1] == 100

    def test_no_extra_pids_still_does_fresh_scan(self) -> None:
        mock_proc = mock.MagicMock()
        mock_proc.pid = 100
        with mock.patch("os.getpgid", return_value=100), \
             mock.patch("os.killpg"), \
             mock.patch.object(process, "find_all_descendant_pids", return_value=[200]) as mock_find, \
             mock.patch.object(process, "kill_session_stragglers"), \
             mock.patch.object(process, "_kill_remaining_descendants") as mock_kill_desc:
            process._kill_process_group(mock_proc)
            mock_find.assert_called_once_with(100)
    ...
    def test_process_already_dead_still_cleans_descendants(self) -> None:
        """When the process is already dead (OSError from getpgid), descendants are still killed."""
        mock_proc = mock.MagicMock()
        mock_proc.pid = 100
        with mock.patch("os.getpgid", side_effect=OSError("No such process")), \
             mock.patch.object(process, "find_all_descendant_pids", return_value=[]), \
             mock.patch.object(process, "kill_session_stragglers") as mock_session, \
             mock.patch.object(process, "_kill_remaining_descendants") as mock_kill_desc:
            process._kill_process_group(mock_proc, extra_pids={500})
            mock_session.assert_called_once_with(100)
            call_args = mock_kill_desc.call_args
